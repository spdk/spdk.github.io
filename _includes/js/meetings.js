...
function compareHoliday(date, holidayList, zone) {
        ...
        holidayList.forEach(function (element) {
                ...
                if (element[1] == false) {
                        if (localDate.getFullYear() != element[0].getFullYear()) {
                                return;
                        }
                }
                if (localDate.getMonth() != element[0].getMonth()) {
                        return;
                }
                // Any meeting after business hours on the day before a holiday is typically cancelled.
                if (localDate.getDate() != element[0].getDate() && (localDate.getHours < overnightStart || localDate.getDate() != element[0].getDate() - 1)) {
                        return;
                }
                isHoliday = true;
                return;
