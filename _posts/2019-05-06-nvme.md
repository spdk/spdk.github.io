---
layout: post
title:  11.6M Storage I/O Per Second From One Thread
author: Ben Walker
categories: news
---

Yes, you're reading that right. That's **11.6 MILLION** 512B random read I/O
...
driver. We'll show some benchmarks showing SPDK performing more than 11 million
512B random read I/O per second, on real hardware available today, using just a
single thread, all while keeping the average latency per I/O to just **57μs**.
...
That's 11.6 million 512B random read I/Ops. And here's the most unbelievable part -
the average latency of each I/O is **57μs**. Here's the specs for the system
we ran this on. While expensive enough that this probably isn't your home gaming
rig, note that it's composed of commercial, off the shelf products.
It's a regular Xeon server platform with a bunch of NVMe SSDs plugged into it.
...
<tr><td>Storage </td><td> 20x Intel P4800X </td></tr>
...
<tr><td>Turbo Frequency </td><td> 3.9GHz </td></tr>
</tbody>
</table>

Moving the block size up to 4KiB, which is increasingly becoming the standard in
storage, still yields a great result:
...
note, however, that operating system drivers can't assign queue pairs to threads
because the lifetime of the operating system driver is much longer than any
given thread and there may be many more threads on the system than available
...
application, is able to directly assign queue pairs to threads and avoid the
locks entirely.
...
Each NVMe queue doorbell ring, both on the submission and the completion side,
is an MMIO write. MMIO writes are posted, meaning the CPU does not stall waiting
for any acknowledgement that the write made it to the PCIe device. This means
that MMIO writes are much faster than MMIO reads. That said, they still have a
significant cost.
...
have a much deeper queue available. If the CPU generates too many MMIO
operations and overflows that queue, the CPU will stall and wait for a slot at
the end of the queue to open up.
