...
single thread, all while keeping the average latency per I/O to just **67μs**.
...
4KiB random reads at queue depth 32 to each device.

<br/>

That's 10 million 4KiB random read I/Ops. And here's the most unbelievable part
- the average latency of each I/O is **67μs**. Here's the specs for the system
