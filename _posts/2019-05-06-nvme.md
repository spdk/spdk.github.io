...
section above, remember this scenario upon finding a new I/O completion:
...
dereferences that opaque context. It may be better for us to instead loop over
all of the completion queue entries one time, kicking off prefetches to get the
tracker, the callback function, the first cache line in the user's opaque
context, and the first cache line in the request. Then, looping again to
actually call the user's callback for each completion. At low queue depth this
may not help, but if we discover 30-50 completions at a time like we are seeing
in the 10M I/O benchmark, this gives the CPU plenty of time to pre-fetch all of
the right data for the majority of the completions. This is a weird scenario
where an algorithm that is O(2n) may be faster than one that is O(n) due to the
complexities of real hardware.
